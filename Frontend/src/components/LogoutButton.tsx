...
const logoutButtonStates: Record<string, React.CSSProperties> = {
  'default': {
    '--figure-duration': '100',
...
export function LogoutButton({ onLogout, disabled, className = '', variant = 'light' }: LogoutButtonProps) {
  ...
  const handleClick = () => {
    ...
    addTimeout(() => {
      ...
      setState('walking2')
      addTimeout(() => {
        setIsFalling(true) // Triggers shake/flash
        setState('falling1')
        ...
        addTimeout(() => {
          ...
          }, parseInt((logoutButtonStates['falling2'] as any)['--walking-duration']))

        }, parseInt((logoutButtonStates['falling1'] as any)['--walking-duration']))

      }, parseInt((logoutButtonStates['walking2'] as any)['--figure-duration']))

    }, parseInt((logoutButtonStates['walking1'] as any)['--figure-duration']))
  }
  ...
  return (
    <button
      className={`logoutButton ${variant === 'light' ? 'logoutButton--light' : ''} ${isClicked ? 'clicked' : ''} ${isDoorSlammed ? 'door-slammed' : ''} ${isFalling ? 'falling' : ''} ${className}`}
      style={style}
    ...
    >
      ...
      <svg className="figure" viewBox="0 0 100 100">
        ...
        <g className="leg2">
          ...
          <path className="calf2" d="M29.5 82.3l9.6-10.9c1.3-1.4 3.6-1.5 5.1-.1 1.5 1.4.4 4.9-.9 6.3l-8.5 9.6c-1.3 1.4-3.6 1.5-5.1.1-1.4-1.5-3.5-.2-5z" />
        </g>
      </svg>
